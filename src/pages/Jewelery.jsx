...
const Jewelery = () => {
  ...
  useEffect(() => {
    ...
    setJeweleryCategory(() => products.filter((item) => item.category === "jewelery"));
  ...
  return (
    <div className='grid xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 max-w-6xl p-1 mx-auto space-y-10 space-x-5 mr-5 min-h-[70vh]'>
      {
        jeweleryCategory.map((item) => {
          return (
            <div className="flex flex-col items-center justify-between 
    hover:scale-105 transition duration-300 ease-in gap-1 p-3 mt-10 ml-5 rounded-xl outline bg-slate-800">
      <div>
